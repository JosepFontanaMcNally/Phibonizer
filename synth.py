...
class Instrument:
    ...
    def __init__(self, harmonics=[1], amps=None, attack=0.1, release=0.6, amp_scale=0.1, max_amp=0.8, sample_rate=44100, chunk_size=512):
    ...
    def emit_sound(self, record=False):
        ...
        try:
            while self.playing:
                if (not self.pressed_keys) and (not self.releasing_keys):
                    ...
                    chunk = np.zeros(self.chunk_size)
                    chunk += (self._generate_pressed_sound() + self._generate_releasing_sound())*self.amp_scale
                    chunk = chunk.clip(-self.max_amp, self.max_amp)
            ...
            self.stream.close()
            if record:
                wavfile.write("recording.wav", rate=self.sample_rate, data=recorded_audio.astype(np.float32))
    ...
    def on_press(key):
        if key not in pressed:
            ...
            try:
                ...
                try:
                    if key.name == 'ctrl':
                        Fibonizer2.playing = False
                    elif key.name == 'cmd':
                        Fibonizer2.emit_sound(record=True)
                except:
                    pass
